...
Deno.serve(async (req) => {
  try {
    ...
    // Auto-detect mode from the price type to avoid mismatch errors
    const stripePrice = await stripe.prices.retrieve(price_id);
    const resolvedMode = stripePrice.type === 'recurring' ? 'subscription' : 'payment';

    console.log(`Checkout: price=${price_id}, price_type=${stripePrice.type}, mode=${resolvedMode}`);
    ...
    return corsResponse({
      error: stripeError.message || error.message,
      type: stripeError.type,
      code: stripeError.code,
    }, 500);
  }
