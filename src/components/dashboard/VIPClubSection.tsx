...
import { Crown, Sparkles, Users, Calendar, Gift, Star, ArrowRight } from "lucide-react";
...
const UPCOMING_FEATURES = [
  { icon: Users, label: "Co-Founder Network" },
  { icon: Calendar, label: "Exclusive Events" },
  { icon: Gift, label: "Premium Perks" },
  { icon: Star, label: "Priority Support" },
];

export default function VIPClubSection({ onNavigateToUpgrade: _ }: VIPClubSectionProps) {
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center px-6 py-16 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none select-none">
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 rounded-full bg-[#FFC107]/4 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-2xl mx-auto">
        ...
        <h1 className="text-5xl md:text-6xl font-black text-black dark:text-white mb-5 leading-none tracking-tight">
          Prolify<br />
          <span className="text-[#FFC107]">VIP Club</span>
        </h1>

        <p className="text-base md:text-lg text-black/60 dark:text-white/60 max-w-md leading-relaxed mb-10">
          An exclusive community for elite entrepreneurs — co-founder networking, private events, premium perks, and so much more coming your way.
        </p>
        ...
        <div className="w-full max-w-md p-6 rounded-2xl bg-black dark:bg-white border-2 border-black dark:border-white">
          <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-sm font-bold text-white dark:text-black">
              Your membership is secured
            </p>
          </div>
          <p className="text-xs text-white/60 dark:text-black/60 leading-relaxed">
            The VIP Club experience is being carefully crafted. As a paid member, you&apos;ll get full access as soon as we go live — which will be very soon.
          </p>
        </div>
      </div>
    </div>
